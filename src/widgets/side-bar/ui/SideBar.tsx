...
const ListItem: React.FC<{ children: React.ReactNode, onClick?: React.MouseEventHandler<HTMLElement>, className?: string }> = ({ children, onClick, className }) => {
  return (
...
export const SideBar: React.FC<ISideBarProps> = ({ close, setIsBoardModalOpen, showSidebar }) => {
    ...
    <SideBarContainer close={close}>
      ...
      <SideBarList>
        {boards.map((item, index) => {
          return (
            <ListItem key={index} onClick={() => { dispatch(boardsSlice.actions.setBoardActive({ index })); showSidebar() }} className={item.isActive ? 'active' : ''} >{item.name}</ListItem>
          )
        })}
      </SideBarList>
